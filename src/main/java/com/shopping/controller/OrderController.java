...
public class OrderController {
    ...
    public ResponseEntity order (@RequestBody @Valid OrderDto orderDto, BindingResult bindingResult, Principal principal) {
        ...
        try {
            orderId = this.orderService.order(orderDto, email);
        } catch (Exception e) {
    ...
    public String orderHist(@PathVariable("page") Optional<Integer> page, Principal principal, Model model) {
        ...
        Page<OrderHistDto> ordersHistDtoList = this.orderService.getOrderList(principal.getName(), pageable);
    ...
    public ResponseEntity cancelOrder(@PathVariable("orderId") Long orderId, Principal principal) {
        if (!this.orderService.validateOrder(orderId, principal.getName())) {
